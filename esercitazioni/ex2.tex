...
\item Utilizzare \texttt{python} per graficare la traiettoria, plottando $y$ in funzione di $x$.
  ...
  con il parametro \texttt{usecols=(i,j)}, dove \lstinline!i! e \lstinline!j! sono le colonne da plottare (partendo da $0$, che indica la prima colonna). Nel nostro caso, se volessimo plottare solo la seconda e la terza colonna il comando completo da usare nello script \texttt{python} diventerebbe:\/} \\
...
\begin{lstlisting}[language=Python,numbers=none]
  x, y, vx, vy = np.loadtxt('traiettoria.dat', usecols=(0,1,2,3), unpack=True)
